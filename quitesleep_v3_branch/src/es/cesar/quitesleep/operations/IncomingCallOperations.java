...
import es.cesar.quitesleep.data.mailmessages.SendMail;
...
import es.cesar.quitesleep.tasks.smsmessages.SendSMSThread;
...
public class IncomingCallOperations extends Thread {
	...
	private static final String CLASS_NAME = IncomingCallOperations.class.getName();
	...
	public synchronized void silentIncomingCall () {
		...
		try {								
			...
			if (isInInterval) {
				...
				Log.d(CLASS_NAME, "BloquedContactBean: " + bcBean);
			...
			//If the call isn't in the interval range
				...
				Log.d(CLASS_NAME, "No está en el intervalo");
	...
	private boolean isInInterval (CallLog callLog, Schedule schedule) {
		...
		try {
			...
			Log.d(CLASS_NAME, "now: " + now + "\ttimeNow: " + timeNow);
			//--//
			...
			Log.d(CLASS_NAME, "Incoming call in the interval?: " + isInInterval);
